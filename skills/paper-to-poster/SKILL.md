...
| File | What it is |
|---|---|
| `axxx/config.json` | Single source of truth for the asset release: `owner=AXXX-Institute`, `repo=skills`, `assets_tag=assets-v1`. Change here to retarget. |
| `axxx/theme_tokens.css` | The AXXX `:root` palette (accent `#0689D4`, deep `#053957`, light tints, one semantic red) + Inter as the only family. |
| `axxx/brand.css` | The brand graphics: top accent bar (`.poster::before`), arrow-glyph bullets (`.card li::before` → `images/bullet_arrow.svg`), affiliation-logo header strip. |
| `axxx/apply_theme.py` | Appends one `<style id="axxx-theme">` after the template's own `<style>` (cascade overrides tokens) + injects the Inter `<link>`. **Idempotent.** |
| `axxx/logos.json` | Affiliation registry (AIRI, HSE, Innopolis, FusionBrain) → release filename, Gate-E size class, chip. |
| `axxx/fetch_assets.py` | Downloads the chosen logos from the pinned `assets-vN` release and **commits them into the poster's `images/`** (offline fallback to the bundled `axxx/assets/`). |
...
Because logos are **fetched then committed** (never referenced remotely), the
poster renders offline, deploys to Pages, and passes `measure`'s networkidle gate.
Re-fetching a newer tag is the upgrade path. Rationale: `docs/adr/0002`.
...
- **After a successful build, ask once** (one `AskUserQuestion`):
  *"Add a Pages deploy workflow? GitHub Actions / GitLab CI / No."* If the user
  ...
  and point it at the `poster/` dir (publishes `poster.html` + `images/` + PDF as
  the static site). Default to **not** wiring CI if the user declines.
- Creating remotes, enabling Pages, and pushing are **outward-facing** — confirm
  with the user before doing them; never push silently.
...
6. **The takeaways strip is optional — decide, don't default.** The landscape scaffolds ship with a bottom takeaways strip, but it earns its place only as a genuine 60-second narrative exit (Idea / Method / Result / Practical). If the final column cards already land the conclusion, or it would just restate the body, **delete the whole `.takeaways-strip` block** rather than pad it — a redundant strip is worse than none (portrait templates omit it by design). Same spirit as *"Fill means substance"* below: don't add a block that doesn't carry its weight.
...
**Lone wrapped fragment — display text that breaks to a one-word last line.** The orphan logic extends from stat numbers to *any short display text that wraps* — a title, `.section-title`, banner headline, or one-line takeaway whose break strands a single short token alone on the final line. The worst case is when that lone token is a **word carrying a leading footnote/superscript marker** (a `*`- or `†`-prefixed term): stranded alone and left-aligned, the marker+word reads as a stray symbol or a typo, not a word. `polish` **does** catch this now (`WIDOW`): it measures wrap geometry on `.callout`, `.body-text`, `.caption`, `.section-title`, `.card p`, `.card li` — including each `<br>`-delimited segment — and warns when the last visual line holds exactly one word. Limits (still guard by hand): it skips blocks containing math/figures (`mjx-container`, `img`, `svg`), `white-space: nowrap/pre`, RTL, and running prose over ~220 chars; it counts an `&nbsp;`-glued multiword tail as multiple words (so glue *clears the `WIDOW` warning* — but see the gap caveat below), and a two-word last line is allowed. Fix a `WIDOW` (or a fused-glyph case the gate can't see) by the option that makes the lines **fill naturally**, not merely the one that silences the gate — in this order of preference: (1) **reword — expand or contract the sentence** — first choice for left-aligned prose (`.callout`, `.body-text`, `.caption`): move the break to a natural phrase boundary so the last line carries ≥2 words *and* the line above fills to the margin. Adding one word often does it (e.g. "expressive" → "highly expressive"). (2) **`text-wrap: balance`** on a *centered heading / title* evens the lines so no single word is stranded (the default on centered display text — keep it; never pair `balance` with `text-align: left`, per the wrap rules above). (3) **`&nbsp;` glue** is a **last resort**: it only moves the break point, so on left-aligned prose it trades the one-word orphan for a *ragged short line with a big right-side gap* — itself an artifact `polish` does **not** catch (it counts only last-line words, never how full the line above is). Glue is genuinely right only where a token *must not* open a line: a leading footnote/superscript marker (`…&nbsp;*term`) or a tight stat cell. After any fix, **re-render and look** — a cleared gate is necessary, not sufficient. Never ship a one-word last line, a lone `*` / `†` / `‡` fragment, or a line left half-empty by a glue "fix".
