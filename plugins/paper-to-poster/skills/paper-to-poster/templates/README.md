...
> **AXXX skill: use the `*_axxx.html` variants.** Each layout ships in two forms:
> a neutral upstream scaffold (`*_neutral.html`, kept for provenance) and an
> **AXXX** scaffold (`*_axxx.html`, generated by `axxx/apply_theme.py`) that
> carries the fixed AXXX theme. **Always copy the `_axxx` one** — this skill is
> AXXX-only and does not use a neutral palette. Then run
> `axxx/fetch_assets.py --dest <work-dir>/images` for the affiliation logos.

Each template is a **self-contained** HTML file: no paper content — only TODO placeholders. Copy one to your working dir as `poster.html`, swap TODOs for your content, then iterate via `tools/poster_check.py measure` + `tools/render_preview.py`. (The `_neutral` files carry upstream design tokens; the `_axxx` files carry the AXXX brand. **Do not** hand-edit the `:root` palette in the AXXX skill.)
